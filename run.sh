...
export DBPassword=$(aws secretsmanager get-secret-value --region ${region} --secret-id workshop-v7-db-password --query SecretString --output text)
export DBHostname=$(aws secretsmanager get-secret-value --region ${region} --secret-id workshop-v7-db-hostname --query SecretString --output text)
